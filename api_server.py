...
async def lifespan(app: FastAPI):
    ...
    # --- 初始化自定义模块 ---
    battle_db.init_db() # 初始化对战数据库
    elo_manager.init_db() # 初始化ELO数据库
...
def convert_openai_to_lmarena_payload(openai_data: dict, session_id: str, message_id: str, mode_override: str = None, battle_target_override: str = None) -> dict:
    ...
    # --- DEBUGGING ---
    # 根据用户请求，强制使用已知的、能正常工作的模型ID，以进行测试。
    target_model_id = "f44e280a-7914-43ca-a25d-ecfcc5d48d09"
    logger.warning(f"DEBUG MODE: 强制使用模型ID -> {target_model_id}")
...
async def non_stream_response(request_id: str, model: str):
    ...
    async for event_type, data in _process_lmarena_stream(request_id):
        if event_type == 'content':
            ...
            logger.info(f"NON-STREAM [ID: {request_id[:8]}]: 返回给用户的最终错误响应体: {json.dumps(error_response, ensure_ascii=False)}")
    ...
    logger.info(f"NON-STREAM [ID: {request_id[:8]}]: 返回给用户的最终成功响应体: {json.dumps(response_data, ensure_ascii=False)}")
...
async def get_models():
    """提供兼容 OpenAI 的模型列表。"""
    # Since models.json is removed, return a default or empty list.
    # Returning a minimal list to keep clients happy.
    return {
        "object": "list",
        "data": [
            {
...
async def chat_completions(request: Request):
    ...
    # --- 对战模式处理 ---
    # 检查是否存在 extra_body 并激活了对战模式
    if openai_req.get("extra_body", {}).get("battle_mode_active"):
        logger.info("检测到对战模式请求，进入专用处理流程...")
        result = battle_mode_handler.handle_battle_mode_request(openai_req)
        
        # handle_battle_mode_request 返回一个元组 (TYPE, data)
        result_type, result_data = result
        
        # 处理 'WIN' 或 'ERROR' 指令，它们都将立即返回一个格式化的非流式响应
        if result_type == 'WIN':
            win_data = result_data
            response_content = (
                f"对战结果已成功记录。\n"
                f"RPID: {win_data['rpid']}\n"
                f"--------------------\n"
                f"Assistant A: {win_data['model_a']}\n"
                f"Assistant B: {win_data['model_b']}"
            )
            model_name_for_response = "battle_results"
        elif result_type == 'ERROR':
            response_content = f"[对战模式错误]: {result_data}"
            model_name_for_response = "battle_error"
        else: # result_type == 'GENERATE'
            processed_req, display_model_name = result_data
            openai_req = processed_req
            # 对于生成请求，跳过下面的直接返回逻辑
            model_name_for_response = None

        if model_name_for_response:
            # --- 兼容流式与非流式 ---
            ...
            if is_stream:
                ...
                response_data = format_openai_non_stream_response(
                    response_content,
                    model_name_for_response,
                    response_id
                )
                return Response(content=json.dumps(response_data, ensure_ascii=False), media_type="application/json")
    else:
        # 根据用户要求，禁用非对战模式的普通请求
        logger.warning("检测到非对战模式的普通请求，已根据配置禁用。")
        response_content = "[服务错误]: 普通聊天请求当前已被禁用，请使用对战模式接口。"
        model_name_for_response = "system_error"
        response_id = f"chatcmpl-{uuid.uuid4()}"
        response_data = format_openai_non_stream_response(
            response_content,
            model_name_for_response,
    ...
    # --- 模型与会话ID映射逻辑 ---
    model_name = openai_req.get("model") # 注意：此时的 model_name 可能是真实模型ID
    ...
    try:
        ...
        if is_stream:
            # 返回流式响应
            return StreamingResponse(
                stream_generator(request_id, display_model_name or "default_model"),
                media_type="text/event-stream"
            )
        else:
            # 返回非流式响应
            return await non_stream_response(request_id, display_model_name or "default_model")
    except Exception as e:
        # 如果在设置过程中出错，清理通道
        if request_id in response_channels:
            del response_channels[request_id]
